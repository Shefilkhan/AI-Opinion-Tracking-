...
export function AlertForm({ onSubmit, loading, error }: AlertFormProps) {
  ...
  return (
    <Card className={cardSurface}>
      <CardHeader>
        <CardTitle className="text-white">Create alert rule</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-xs text-slate-500">Alert type</label>
            <select
              value={alertType}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-500">{thresholdLabel}</label>
            <input
              type="number"
              min={1}
              ...
              className={cn("w-full rounded-lg px-3 py-2 text-sm text-white", inputSurface)}
            />
          </div>

          {alertType === "keyword_mention" && (
            <div>
              <label className="mb-1 block text-xs text-slate-500">Keyword</label>
              <input
                type="text"
                value={keyword}
          ...
          <Button
            type="submit"
            disabled={loading}
            className="w-full gap-2 bg-gradient-to-r from-blue-600 to-violet-600 text-white"
          >
